            ...
            ZStack {
                ...
                VStack {
                    VStack {
                        ...
                        if !manager.gameOver {
                            if manager.player == .player1 && manager.turnPhase == .move {
                                Text("Blue Move Phase")
                                    .bold()
                                    .font(.system(size: 40))
                                    .foregroundStyle(Color.white)
                            } else if manager.player == .player1 && manager.turnPhase == .relocate {
                                Text("Blue Relocate Phase")
                                    .bold()
                                    .font(.system(size: 40))
                                    .foregroundStyle(Color.white)
                            } else if manager.player == .player2 && manager.turnPhase == .move {
                                Text("Red Move Phase")
                                    .bold()
                                    .font(.system(size: 40))
                                    .foregroundStyle(Color.white)
                            } else if manager.player == .player2 && manager.turnPhase == .relocate {
                            ...
                            ForEach(0..<(magicNumber + 1), id: \.self) { row in
                                HStack {
                                    ForEach(0..<(magicNumber + 1), id: \.self) { column in
                                        BoardSquare(manager: $manager, position: Position(row: row, column: column))
                                    }
                                }
                            }
                        }
...
#Preview {
    ...
    BoardTwo(manager: $manager, magicNumber: 7)
}
